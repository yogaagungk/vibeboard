...
function openNewCardModal(colId) {
  ...
  const ncAgentMount = document.getElementById('nc-agent-mount');
  ncAgentMount.innerHTML = '';
  ncAgentMount._agentSelect = vbSelect({
    options: buildAgentOptions(),
    value: '',
    placeholder: 'Select agent',
    ariaLabel: 'Agent',
    onChange: value => {
      document.getElementById('nc-agent-warning').style.display = value ? '' : 'none';
      updateModelDropdown('nc', value);
    },
  });
  ncAgentMount.appendChild(ncAgentMount._agentSelect.el);
...
function getModalAgent() {
  const mount = document.getElementById('card-agent-mount');
  return mount?._agentSelect?.getValue() || undefined;
}
